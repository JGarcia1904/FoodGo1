...
<div class="container">
 ...
 if (isset($_GET['msg3']) == "delete") {
          ...
          <div class="container">
            <div class="row">
              <div class="col-md-12 text-center">
                <form action="reportes1.php" method="post" accept-charset="utf-8">
                  <div class="row">
                    <div class="col">
                      <span>Desde:</span>
                      <input type="date" max="<?=date('Y-m-d')?>" name="fecha_ingreso" class="form-control"  placeholder="Fecha de Inicio" required>
                    </div>
                    <div class="col">
                      <span>Hasta:</span>  
                      <input type="date" max="<?=date('Y-m-d')?>" name="fechaFin" class="form-control" placeholder="Fecha Final" required>
                    </div>
                    <div class="col">
  ...
  <table class="table table-hover" id="tableEmpleados">
    ...
    <tbody>
        <?php 
          $deliverys = $deliveryObj->consulta_registros(); 
          foreach ($deliverys as $delivery) {
        ?>
        <tr>
          <td><?php echo $delivery['cedula'] ?></td>
          <td><?php echo $delivery['nombre'] ?></td>
          <td><?php echo $delivery['productos'] ?></td>
          <td><?php echo $delivery['monto'] ?>$</td>
          <td><?php echo $delivery['fechatransferencia'] ?></td>
          <td><?php echo $delivery['direccion'] ?></td>
          <td><?php echo $delivery['telefono'] ?></td>
          <td><?php echo $delivery['zona'] ?></td>
          <!-- <td>
...
$("#filtro").on("click", function(e){ 
  ...
  if(f_ingreso !="" && f_fin !=""){
    $.post("filtro1.php", {f_ingreso, f_fin}, function (data) {
      $("#registro").hide();
